...
Split into a Qt-free orchestration function (`plan_views`, unit-testable headlessly)
and a thin GUI docking wrapper (`launch_block`). Constructing a real `MainViewer` hangs
in a headless test process, so all automated coverage targets `plan_views`; the thin
`launch_block` docking loop is exercised by the Task 12 manual run.
...
def plan_views(block_path: Path, session: Session, cfg: DictConfig) -> list[ViewPlan]:
    ...
    exactly once, and builds one source per attachment.
...
def launch_block(block_path: Path, session: Session, cfg: DictConfig) -> MainViewer:
    """Build and populate a MainViewer for one block from a session (thin Qt wrapper)."""
